...
const DEFAULT_VALUES = FILTER_CONFIG.reduce((acc, config) => {
  acc[config.key] = config.defaultValue; //acc means accumulator
  return acc;
}, {});
  ...
  const applyFilters = async (newValues) => {
    ...
    try {
      ...
      await new Promise((resolve) => {
        ...
        setTimeout(resolve, 50); //50 means 50millisecs
      });
    } catch (error) {
